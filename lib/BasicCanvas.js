...
export const css = (s, ...exps) => s
  .map((e, i) => [e, exps[i]])
  .reduce((acc, val) => acc.concat(val), [])
  .join('');  // Extremely slow, exist to trick editors in to CSS higlighting.
...
class Canvas {
  constructor(elem) {
    this.elem = elem;
    this.current_width = this.elem.width;
    this.current_height = this.elem.height;
