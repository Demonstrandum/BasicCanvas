...
// --> Try to make `use()` and `type()` functions global.
let _use;
...
export const plain = (s, ...exps) => s
  ...
  .join('');  // Extremely slow, exist to trick editors in to highlighting.
  ...
  background(c = this.fill, clear = false) {
    if (clear) {
      this.context.clearRect(
        Math.sign(this.stretch[0]) * this.corner.x, Math.sign(this.stretch[1]) * this.corner.y,
        Math.sign(this.stretch[0]) * -this.corner.x + this.width, Math.sign(this.stretch[1]) * -this.corner.y + this.height
      );
    }
