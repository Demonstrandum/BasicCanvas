...
class ColorObj {
  ...
  toString() { return `rgba(${this.rgb.join(', ')}, ${this.a/255})`; }
  valueOf() { return this.toString(); }
}

export const Point = (x, y) => { return new PointObj(x, y) };
export const Color = (r, g=-1, b=-1, a=255, type='rgba') => {
  if (b < 0 && g > 0)
    a = g;
  if (b < 0)
    [g, b] = [r, r];
...
class Shape {
  ...
  rect(point, w, h, fill=this.canvas.fill, stroke=this.canvas.stroke) {
    this.style(fill=fill, stroke=stroke)
    let c = this.canvas.context;
  ...
  fill(color=this.canvas.fill) {
    ...
    // redraws all vertices, SLOW and BAD, SAD!
    let c = this.canvas.context;
    c.moveTo(...this.vertices[0]);
    for (let vertex of this.vertices) c.lineTo(...vertex);
    c.closePath();
...
class Canvas {
  ...
  get width() { return this.canvas_elem.width; }
  get height() { return this.canvas_elem.height; }
  ...
  color(point, other=null) {
    if (!other) return Color(...this.context.getImageData(point.x, point.y, 1, 1).data);
    let img_d = this.context.createImageData(1,1)
    let d = img_d.data;
  ...
  point(point, color=this.stroke) { return this.color(point, color); }

  shape(name, construction) {
    if (!name) name = `ImplicitShape${Object.keys(this.shapes).length}`;
    this.shapes[name] = {
  ...
  update_frame(canvas) {
    canvas.shapes = {};
    (canvas.update)();
    window.requestAnimationFrame((t) => { canvas.update_frame(canvas) });
  }
  ...
  loop(update) {
    ...
    this.update = update;
    let that = this
    window.requestAnimationFrame((t) => { that.update_frame(that) });
  }
}
